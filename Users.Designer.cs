...
namespace ClinicManagement
{
    public partial class Users : Form
    {
        ...
        // Panels
        Panel pnlContent;
        Panel pnlIdentity, pnlAccount, pnlActions, pnlGrid;
        ...
        // Identity
        RoundedTextBox txtUserID, txtName, txtFatherName, txtEmail;
        
        // Account details
        RoundedTextBox txtUsername, txtPassword;
        Label lblTogglePassword;
        ComboBox cmbStaffId, cmbStaffName;
        ComboBox cmbRole, cmbStatus;

        // Buttons
        Button btnSave, btnNew, btnEdit, btnDelete, btnRefreshGrid;
        Button btnUploadProfile;
        DataGridView dgvUsers;
        PictureBox pbProfile;
        ...
        private void InitializeComponent()
        {
            ...
            // ================ IDENTITY CARD =================
            pnlIdentity = CreateModernCard("👤  User Identity");
            ...
            pbProfile.Paint += (s, e) =>
            {
                ...
                using (GraphicsPath path = new GraphicsPath())
                {
                    ...
                    using (Pen pen = new Pen(Color.LightGray, 1)) e.Graphics.DrawEllipse(pen, 0, 0, pbProfile.Width - 1, pbProfile.Height - 1);
                }
            };
            ...
            btnUploadProfile = new Button
            {
                ...
                Location = new Point(140, yOffset + 35), // Next to the picture
                BackColor = surfaceColor,
            ...
            btnUploadProfile.FlatAppearance.BorderSize = 1;
            btnUploadProfile.FlatAppearance.BorderColor = Color.LightGray;

            pnlIdentity.Controls.Add(pbProfile);
            pnlIdentity.Controls.Add(btnUploadProfile);

            yOffset += 120; // Push down existing fields

            txtUserID = AddModernTextBox(pnlIdentity, "User ID", yOffset, "Auto-generated");
            txtUserID.Inner.ReadOnly = true;
            txtUserID.Inner.BackColor = Color.FromArgb(245, 248, 250);
            yOffset += FIELD_SPACING;

            cmbStaffId = AddModernComboBox(pnlIdentity, "Staff ID", yOffset, null);
            yOffset += FIELD_SPACING;

            cmbStaffName = AddModernComboBox(pnlIdentity, "Staff Name", yOffset, null);
            yOffset += FIELD_SPACING;

            txtName = AddModernTextBox(pnlIdentity, "Full Name", yOffset, "e.g Ali Raza");
            yOffset += FIELD_SPACING;

            txtFatherName = AddModernTextBox(pnlIdentity, "Father Name", yOffset, "e.g Muhammad Raza");
            yOffset += FIELD_SPACING;

            txtEmail = AddModernTextBox(pnlIdentity, "Email", yOffset, "user@system.com");
            yOffset += FIELD_SPACING;

            // ================ ACCOUNT CARD =================
            pnlAccount = CreateModernCard("🔐  Account Details");
            yOffset = 58;

            txtUsername = AddModernTextBox(pnlAccount, "Username", yOffset, "e.g ali.raza");
            yOffset += FIELD_SPACING;

            txtPassword = AddModernTextBox(pnlAccount, "Password", yOffset, "••••••••");
            txtPassword.Inner.PasswordChar = '•';
            ...
            lblTogglePassword = new Label
            {
                ...
                Cursor = Cursors.Hand,
                AutoSize = true,
                Location = new Point(pnlAccount.Width - 120, yOffset + 12),
                Anchor = AnchorStyles.Top | AnchorStyles.Right
            };
            pnlAccount.Controls.Add(lblTogglePassword);
            lblTogglePassword.BringToFront();

            yOffset += FIELD_SPACING;

            cmbRole = AddModernComboBox(pnlAccount, "System Role", yOffset, new string[] { "Admin", "Receptionist", "Doctor", "Pharmacist" });
            yOffset += FIELD_SPACING;

            cmbStatus = AddModernComboBox(pnlAccount, "Account Status", yOffset, new string[] { "Active", "Inactive" });
            yOffset += FIELD_SPACING;
            ...
            int btnHeight = 48;
            btnSave = CreateModernButton("💾  Save User", successColor, 200, btnHeight);
            ...
            btnDelete = CreateModernButton("🗑️  Delete User", dangerColor, 200, btnHeight);
            btnRefreshGrid = CreateModernButton("🔄  Refresh Grid", accentColor, 200, btnHeight);

            pnlActions.Controls.AddRange(new Control[] { btnSave, btnNew, btnEdit, btnDelete, btnRefreshGrid });

            // ================ DATA GRID CARD =================
            pnlGrid = CreateModernCard("📋  System Users");
            dgvUsers = CreateModernDataGridView();
            dgvUsers.Location = new Point(CARD_PADDING, 55);
            pnlGrid.Controls.Add(dgvUsers);
            dgvUsers.AutoGenerateColumns = true;

            // ================ ADD ALL CONTROLS =================
            pnlContent.Controls.AddRange(new Control[] { pnlIdentity, pnlAccount, pnlActions, pnlGrid });
        ...
        private void LayoutContent() {
            ...
            int cardGap = 16;
            int topRowHeight = 520; // Increased for better clearance
            ...
            pnlIdentity.SetBounds(CONTENT_PADDING, CONTENT_PADDING, basicMedicalWidth, topRowHeight);
            pnlAccount.SetBounds(CONTENT_PADDING + basicMedicalWidth + cardGap, CONTENT_PADDING, basicMedicalWidth, topRowHeight);
            ...
            dgvUsers.SetBounds(10, 55, pnlGrid.Width - 20, pnlGrid.Height - 70);
            ...
            UpdateCardSeparator(pnlIdentity); UpdateCardSeparator(pnlAccount); UpdateCardSeparator(pnlActions); UpdateCardSeparator(pnlGrid);
        }
        ...
        private void LayoutActionButtons() {
            ...
            btnRefreshGrid.SetBounds(CARD_PADDING, btnStartY + btnGapY * 2, btnWidth * 2 + btnGapX, btnHeight);
        }
        ...
        private Panel CreateSearchBox() {
            ...
            txtSearch = new TextBox { Text = "Search users...", Location = new Point(52, 11), Size = new Size(330, 24), BorderStyle = BorderStyle.None, BackColor = Color.FromArgb(249, 250, 251), ForeColor = Color.Gray, Font = new Font("Segoe UI", 10) };
            txtSearch.GotFocus += (s, e) => { if (txtSearch.Text == "Search users...") { txtSearch.Text = ""; txtSearch.ForeColor = textPrimary; } };
            txtSearch.LostFocus += (s, e) => { if (string.IsNullOrWhiteSpace(txtSearch.Text)) { txtSearch.Text = "Search users..."; txtSearch.ForeColor = Color.Gray; } };
        ...
        private Panel CreateTipsPanel() {
            ...
            Label l1 = new Label { Text = "💡 Security Tips", Font = new Font("Segoe UI Semibold", 11, FontStyle.Bold), ForeColor = secondaryColor, Location = new Point(16, 14), AutoSize = true };
            Label l2 = new Label { Text = "• Use complex passwords for administrative user accounts\n• Ensure correct system roles are assigned for access control\n• Regularly audit and deactivate unused user credentials", Font = new Font("Segoe UI", 9), ForeColor = textSecondary, Location = new Point(16, 40), AutoSize = true };
